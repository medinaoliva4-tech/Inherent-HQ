# Playbook — Traducción del slot a intención creativa
...
- **③ Marketing** emite `funcion` + `temperatura` + `awareness` — porque diseña un **sistema**.
...
> Si un slot parece mal clasificado, se devuelve a ③ Marketing — no se corrige acá.
...
|---|---|
| `funcion` · `temperatura` · `pilar` · `formato` · `canal` · `objetivo_del_slot` · `traza_a_must_be_true` · `semana` | ③ calendario |
| `awareness` | la especificación de tipos de pieza de ③ Marketing (especificación de piezas) |
| Función única y **qué NO se hace** por canal | el plan por canal de ③ Marketing |
...
Es normal: son **dos ejes independientes** (③ Marketing (temperatura y consciencia) — la función la decide el problema
...
③ Marketing es **específico** (*"Responder objeción #1"*, *"Ocupar el CEP de martes 18-20h"*).
...
El calendario de ③ Marketing trabaja en **semanas** (`semana` + `frecuencia`), no en días.

| | Quién decide |
|---|---|
| La **semana**, el canal, la frecuencia | 🛑 **③ Marketing.** No se mueven |
| El **día concreto** dentro de esa semana | Creative, respetando la `frecuencia` del slot y la estacionalidad de la ② ingeniería inversa §1.2e |
...
**Los 5 estados, con las etiquetas exactas de ② Estrategia (estados de consciencia)** — se copian literales, no se abrevian:
...
El funnel de adquisición tiene tres etapas; la temperatura de ③ Marketing tiene **cuatro** estados —
...
dice Marca, se devuelve a ③ Marketing.
...
- [ ] `awareness` copiado de la especificación de tipos de pieza de ③ Marketing, **con la etiqueta literal de ③ Marketing**
...
- [ ] La función del canal coincide con el plan por canal de ③ Marketing, y su **"qué NO se hace acá"** está leído
- [ ] Si algo no encaja: **devuelto a ③ Marketing**, no corregido acá
...
|---|---|
| Falta `traza_a_must_be_true` | **No produce la fila.** Devuelve el slot a ③ Marketing |
| Falta `awareness` | Lo busca en la especificación de tipos de pieza de ③ Marketing. Si no está: `⚠️ SIN DATOS` y se declara que el ángulo queda con confianza reducida |
...
| Falta `formato` | Se propone desde el plan por canal de ③ Marketing (y si no está ahí, de `toolkit/07-plataformas.md`) y se marca `🟡 propuesto por Creative` para que ③ Marketing lo confirme |
| El `goal_del_arte` contradice el `objetivo_del_slot` | Se devuelve el slot. **No se elige uno de los dos** |
| **Dos canales distintos con la misma `funcion`** | Devuelve: ③ Marketing define **una función distinta por canal** (*"si dos canales hacen lo mismo, uno sobra"*) |
| **Un canal con una `funcion` que el plan por canal de ③ Marketing no le asigna** | Devuelve el slot: el calendario contradice el sistema |
